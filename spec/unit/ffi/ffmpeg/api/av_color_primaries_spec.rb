require 'spec_helper'
require 'ffi/ffmpeg'
require 'ffi/ffmpeg/api/av_color_primaries'


describe 'FFI::FFmpeg::AVColorPrimaries' do
  subject { FFI::FFmpeg::AVColorPrimaries }

  it 'defines an enum' do
    subject.should be_a FFI::Enum
  end

  specify { subject[:bt709].should == 1 }
  specify { subject[:unspecified].should == 2 }
  specify { subject[:bt470m].should == 4 }
  specify { subject[:bt470bg].should == 5 }
  specify { subject[:smpte170m].should == 6 }
  specify { subject[:smpte240m].should == 7 }
  specify { subject[:film].should == 8 }
  specify { subject[:nb].should == 9 }
end
